...
const Cart = () => {
    ...
    let numberOfGoods = 0
    cart.map((product) => {
        numberOfGoods += product.count
        return numberOfGoods
    })

    useWindowScrollToTop()
    return (
        <section className="cartPage bg-white md:bg-gray-200" style={{ padding: '70px 15px 10px 15px' }}>
            {cart.length === 0 ?
                <CartEmpty />
                :
                // cart is not empty
                <div className="flex flex-col-reverse lg:flex-row md:items-start gap-[.75rem]">
                    <div className="showProductsCard bg-white flex-1">
                        ...
                        <div className="flex flex-col gap-[.5rem] md:gap-0">
                            {cart.map((product, index) => {
                                return <CartProductCard key={index} image={product.image} title={product.title} count={product.count} id={product.id} price={product.price} />
                            })}
                        </div>
                        <div className="flex items-center justify-between gap-[1.5rem]">
                            ...
                            onClick={() => removeAllProductFromCart()}>Delete All</button>
                        <p
